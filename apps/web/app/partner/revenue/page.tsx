...
export default function PartnerRevenuePage() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        setLoading(false); // Redirect logic here if needed
      }
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    const loadRevenue = async () => {
      ...
      try {
        // ✅ PASS PARTNER ROLE AND ID
        const res = await fetch(
          `/api/revenue?role=partner&partnerId=${user.uid}&${query}`
        );
  ...
  const groupedData = useMemo(() => {
    ...
    rawData.bookings.forEach((booking: any) => {
      ...
      groups[key].totalEarnings += booking.yourCut; // Using 'totalEarnings' instead of 'totalCommission'
  ...
  const generatePDF = (
    ...
    autoTable(doc, {
      startY: 40,
      head: [["Total Sales", "My Earnings", "Transactions"]],
      body: [
        [
    ...
    autoTable(doc, {
      head: [["Date", "Property", "Type", "Guest", "Amount", "Payout"]],
      body: rows,
      startY: 70,
      theme: "grid",
      styles: { fontSize: 9 },
  ...
  const handleMainDownload = (type: "pdf" | "excel") => {
    if (!rawData?.bookings) return;
    const fName = `My_Revenue_${new Date().toISOString().slice(0, 10)}`;
  ...
  const handleSingleDownload = (group: any, type: "pdf" | "excel") => {
    ...
    if (type === "excel") generateExcel(group.transactions, fileName);
      ...
      const summary = {
        ...
        netRevenue: group.totalEarnings,
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-black p-4 md:p-6 lg:p-10 font-sans pb-20">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end mb-8 gap-6">
        <div className="w-full xl:w-auto">
          <Link
            href="/partner"
            className="text-sm font-bold text-gray-400 hover:text-rose-600 transition-colors mb-2 inline-flex items-center gap-1"
          >
          ...
          <p className="text-gray-500 mt-1 text-sm md:text-base">
            Track your property revenue and payouts.
          </p>
        </div>
        ...
        <div className="w-full xl:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-white dark:bg-gray-900 p-2 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm">
          <div className="relative w-full sm:w-auto">
            ...
            <select
              ...
              className="w-full sm:w-auto pl-9 pr-8 py-2.5 bg-gray-50 dark:bg-gray-800 text-sm font-bold text-gray-700 dark:text-gray-200 outline-none cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-colors appearance-none border border-transparent focus:border-rose-500"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 mb-8">
        ...
        <StatCard
          ...
          value={rawData?.summary.bookingsCount}
        ...
        <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-gray-900 sticky top-0 z-10">
          ...
          <div className="relative w-full sm:w-auto" ref={mainDownloadRef}>
            <button
              ...
              className="w-full sm:w-auto justify-center flex items-center gap-2 text-sm font-bold text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 px-4 py-2.5 rounded-xl transition-all border border-gray-200 dark:border-gray-700"
            >
            ...
            {showMainDownload && (
              <div className="absolute right-0 top-12 w-full sm:w-56 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl z-50 p-2 animate-in fade-in slide-in-from-top-2">
                <button
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-gray-50/50 dark:bg-gray-800/50 text-xs font-bold uppercase text-gray-400 tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 text-right">My Payout</th>
              ...
              {currentData.map((group: any) => (
                <>
                  <tr
                    ...
                    className={`cursor-pointer transition-all duration-200 group ${expandedRow === group.name ? "bg-rose-50/30 dark:bg-rose-900/10" : "hover:bg-gray-50 dark:hover:bg-gray-800/40"}`}
                  >
                    <td className="pl-6 py-5 text-gray-400 group-hover:text-rose-500">
                    ...
                    <td className="px-6 py-5 text-right relative">
                      <button
                        onClick={(e) => {
                          ...
                          setActiveDownloadRow(
                            activeDownloadRow === group.name ? null : group.name
                          );
                        }}
                        className="p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full text-gray-400 hover:text-rose-600 transition-colors relative z-10"
                      >
                      ...
                      {activeDownloadRow === group.name && (
                        <div
                          ref={rowMenuRef}
                          className="absolute right-8 top-8 w-48 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl z-20 p-2 animate-in fade-in zoom-in-95"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button
                            onClick={() => handleSingleDownload(group, "excel")}
                            className="dropdown-item text-green-700 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/20"
                          >
                            <FileSpreadsheet size={16} /> Excel Report
                          </button>
                          <button
                            onClick={() => handleSingleDownload(group, "pdf")}
                            className="dropdown-item text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20"
                          >
                                  ...
                                  >
                                    <td className="py-3 px-6 text-gray-500 font-medium">
                                      ...
                                      <span className="text-xs">
                                        {format(new Date(trx.date), "hh:mm a")}
                                      </span>
                                    </td>
