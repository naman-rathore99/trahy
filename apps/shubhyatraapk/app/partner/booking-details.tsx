...
export default function PartnerBookingDetails() {
  ...
  useEffect(() => {
    ...
    const fetchBooking = async () => {
      try {
        ...
        const res = await api.get(`/api/partner/bookings/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data.booking) setBooking(res.data.booking);
        else setErrorMsg("Booking not found.");
      } catch (e: any) {
        setErrorMsg(e.response?.data?.error || "Failed to load booking.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCheckIn = () => {
    Alert.alert("Confirm Check-In", "Mark this guest as checked in?", [
  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-[#09090B]">
      ...
      <View className="h-72 bg-gray-900 relative">
        <Image
          source={{
            uri: "https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=1000",
          }}
        ...
        <Text className="absolute bottom-6 left-6 text-white text-3xl font-bold">
          {booking.listingName || "Hotel Stay"}
        </Text>
      </View>
      ...
      <View className="absolute bottom-6 left-6 right-6">
        <TouchableOpacity
          ...
          className="w-full bg-[#FF5A1F] h-16 rounded-2xl items-center justify-center flex-row gap-2 shadow-lg shadow-orange-500/30"
        >
